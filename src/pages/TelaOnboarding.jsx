...
function CampoValor({ icone, label, valor, onChange, destaque }) {
  const [display, setDisplay] = useState(valor ? Number(valor).toLocaleString('pt-BR') : '')
  function handle(e) {
    const d = e.target.value.replace(/\D/g,'')
    setDisplay(d ? Number(d).toLocaleString('pt-BR') : '')
    onChange(Number(d) || 0)
  }
  return (
    <div style={{
      ...
      <label style={{ fontSize:14, fontWeight:600, color:'#525252', flex:1, cursor:'pointer' }}>{label}</label>
...
function SecaoDespesas({ titulo, icone, cor, campos, valores, onChange }) {
  const total = Object.values(valores).reduce((a,b) => a+(Number(b)||0), 0)
  return (
    <div style={{ marginBottom:8 }}>
      <div style={{
        display:'flex', alignItems:'center', justifyContent:'space-between',
        padding:'10px 0', marginBottom:8,
        borderBottom:`2px solid ${cor}20`,
      }}>
        <div style={{ display:'flex', alignItems:'center', gap:8 }}>
        ...
        {total > 0 && (
          <span style={{ fontSize:14, fontWeight:800, color: cor }}>{formatarMoeda(total)}</span>
        )}
      </div>
      <div style={{ display:'flex', flexDirection:'column', gap:8 }}>
        {campos.map(c => (
          <CampoValor
            key={c.key} icone={c.icone} label={c.label}
      ...
      <main style={{ flex:1, maxWidth:560, margin:'0 auto', padding:'32px 24px 24px', width:'100%' }} key={etapa}>
          ...
          <div style={{ animation:'slideIn 0.3s ease forwards' }}>
            ...
            <div style={{ display:'flex', flexDirection:'column', gap:10 }}>
              {[
                ...
                <OpcaoCard key={o.valor} {...o}
                  selecionado={dados.problema === o.valor}
                  onSelect={v => setDados(d => ({...d, problema:v}))}
                />
              ))}
            </div>
          </div>
        ...
        {etapaAtual === 'receita' && (
          <div style={{ animation:'slideIn 0.3s ease forwards' }}>
            ...
            <p style={{ fontSize:15, color:'#737373', marginBottom:28 }}>
              Some todas as fontes: salário, freelas, aluguéis. Use o valor líquido.
            </p>
            <div style={{ position:'relative', marginBottom:10 }}>
              ...
              <input
                ...
                onChange={e => { const d=e.target.value.replace(/\D/g,''); setDados(prev=>({...prev,receita:Number(d)||0})) }}
                placeholder="0"
                style={{
                  width:'100%', padding:'20px 20px 20px 68px',
                  background:'#fff', border:'2px solid #e5e5e5', borderRadius:14,
                  fontSize:36, fontWeight:900, color:'#0f0f0f', outline:'none', letterSpacing:'-0.02em',
                  transition:'border-color 0.2s, box-shadow 0.2s',
                }}
            ...
            <div style={{ display:'flex', flexDirection:'column', gap:10 }}>
              {[
                ...
                <OpcaoCard key={o.valor} {...o}
                  selecionado={dados.tipoRenda === o.valor}
                  onSelect={v => setDados(d=>({...d,tipoRenda:v}))}
                />
              ))}
            </div>
          </div>
        ...
        {etapaAtual === 'despesas' && (
          <div style={{ animation:'slideIn 0.3s ease forwards' }}>
            ...
            <p style={{ fontSize:15, color:'#737373', marginBottom:20 }}>
              Preencha o que se aplica. Deixe em zero o que não tiver. O total vai aparecendo ao vivo.
            </p>
            ...
            <div style={{
                ...
                {dados.receita > 0 && (
                  <div style={{ display:'flex', alignItems:'center', gap:8 }}>
                    <div style={{ width:80, height:6, background:'#e5e5e5', borderRadius:3, overflow:'hidden' }}>
                      <div style={{
                        height:'100%', borderRadius:3, background: corPercentual,
                        width:`${Math.min(percentual,100)}%`, transition:'width 0.4s ease',
                      }}/>
                    </div>
              ...
              {percentual > 90 && (
                <div style={{ marginTop:10, background:'#fef2f2', borderRadius:8, padding:'8px 12px', fontSize:13, color:'#991b1b', fontWeight:600 }}>
                  ⚠️ Seus gastos comprometem {percentual}% da renda — isso é crítico.
                </div>
              )}
              {percentual > 80 && percentual <= 90 && (
                <div style={{ marginTop:10, background:'#fffbeb', borderRadius:8, padding:'8px 12px', fontSize:13, color:'#92400e', fontWeight:600 }}>
                  ⚡ Atenção: {percentual}% da renda comprometida. Pouca margem de segurança.
                </div>
              )}
            </div>
            ...
            <div style={{ marginBottom:8 }}>
              ...
              <CampoValor
                icone="💳"
                label="Total da fatura mensal (tudo que você coloca no crédito)"
                valor={dados.cartao}
                onChange={v => setDados(d=>({...d,cartao:v}))}
                destaque={true}
              />
