...
function ProjecaoAnual({ receita, totalGastos, risco }) {
  ...
  return (
    <div ref={ref} style={{
      ...
      {deficitario && (
        <div style={{ position: 'absolute', inset: 0, background: 'radial-gradient(ellipse at 50% 0%, rgba(220,38,38,0.1) 0%, transparent 65%)', pointerEvents: 'none' }}/>
      )}
      <div style={{ marginBottom: 20, position: 'relative' }}>
        <div style={{ fontSize: 11, fontWeight: 800, color: deficitario ? '#f87171' : '#f97316', letterSpacing: '0.1em', textTransform: 'uppercase', marginBottom: 6 }}>
          {deficitario ? '🚨 ALERTA — ' : '📊 '}Projeção 12 Meses
        </div>
        <h3 style={{ fontSize: 17, fontWeight: 800, color: deficitario ? '#ffffff' : '#0f0f0f', lineHeight: 1.3 }}>
          {deficitario ? 'Esse ciclo vai acumular dívidas ao longo do ano' : 'Evolução do seu caixa ao longo do ano'}
        </h3>
      </div>
        ...
        {mesesDados.map((m, i) => {
          ...
          return (
            <div key={i} style={{ flex: 1, display: 'flex', gap: 1, alignItems: 'flex-end', height: '100%' }}>
              <div style={{ flex: 1, background: deficitario ? '#1f2937' : '#d1d5db', borderRadius: '2px 2px 0 0', height: `${hR}%`, transition: `height ${0.4 + i * 0.04}s cubic-bezier(0.4,0,0.2,1)` }}/>
              <div style={{ flex: 1, background: positivo ? '#f97316' : '#dc2626', borderRadius: '2px 2px 0 0', height: `${hG}%`, transition: `height ${0.5 + i * 0.04}s cubic-bezier(0.4,0,0.2,1)`, boxShadow: !positivo ? '0 0 6px rgba(220,38,38,0.6)' : 'none' }}/>
            </div>
          )
        })}
      </div>
      <div style={{ display: 'flex', gap: 3, marginBottom: 16 }}>
        {mesesDados.map((m, i) => (
          <div key={i} style={{ flex: 1, textAlign: 'center', fontSize: 9, fontWeight: 600, color: deficitario ? '#4b5563' : '#a3a3a3' }}>{m.mes}</div>
        ))}
      </div>
      <div style={{ display: 'flex', gap: 16, marginBottom: 16 }}>
        {[{ cor: deficitario ? '#1f2937' : '#d1d5db', label: 'Receita' }, { cor: deficitario ? '#dc2626' : '#f97316', label: 'Gastos' }].map(l => (
      ...
      <div style={{ display: 'flex', gap: 10, marginBottom: 10 }}>
        <div style={{ flex: 1, borderRadius: 12, padding: '14px 12px', background: deficitario ? '#111827' : '#f9f8f6', border: `1px solid ${deficitario ? '#1f2937' : '#e5e5e5'}` }}>
        ...
        <div style={{ flex: 1, borderRadius: 12, padding: '14px 12px', background: deficitario ? '#1a0808' : '#fff5f5', border: `1px solid ${deficitario ? '#7f1d1d' : '#fecaca'}` }}>
      ...
      <div style={{ background: deficitario ? '#1a0808' : '#f0fdf4', border: `2px solid ${deficitario ? '#dc2626' : '#bbf7d0'}`, borderRadius: 14, padding: '18px 16px', display: 'flex', justifyContent: 'space-between', alignItems: 'center', boxShadow: deficitario ? '0 0 24px rgba(220,38,38,0.4)' : 'none' }}>
        <div>
          <div style={{ fontSize: 12, fontWeight: 800, letterSpacing: '0.06em', marginBottom: 4, color: deficitario ? '#f87171' : '#16a34a' }}>
        ...
        <div style={{ fontSize: 'clamp(20px,5vw,26px)', fontWeight: 900, color: deficitario ? '#f87171' : '#16a34a', letterSpacing: '-0.02em' }}>
          {deficitario ? '-' : '+'}{formatarMoeda(Math.abs(saldoAnual))}
        </div>
      </div>
      {deficitario && Math.abs(saldoAnual) > receita * 2 && (
        <div style={{ marginTop: 10, background: '#7f1d1d', borderRadius: 10, padding: '12px 16px', display: 'flex', gap: 10 }}>
          <span style={{ fontSize: 18, flexShrink: 0 }}>🚨</span>
          <p style={{ fontSize: 13, color: '#fca5a5', lineHeight: 1.5, fontWeight: 600 }}>
            Em 12 meses esse ciclo acumula um déficit equivalente a {Math.round(Math.abs(saldoAnual) / receita)} meses de renda.
          </p>
        </div>
      )}
    </div>
...
function CardAnaliseRenda({ analiseRenda, receita }) {
  const { tipoProblema, titulo, corpo, cor, rendaMinimaIdeal, precisaAumentarRenda } = analiseRenda
  return (
    <div style={{ background: tipoProblema === 'organizacao' ? '#fffbeb' : '#fff', border: `2px solid ${cor}`, borderRadius: 16, padding: '22px 20px', marginBottom: 16, boxShadow: precisaAumentarRenda ? `0 4px 20px ${cor}25` : 'none' }}>
      <div style={{ fontSize: 11, fontWeight: 800, letterSpacing: '0.1em', textTransform: 'uppercase', color: cor, marginBottom: 10 }}>
        {precisaAumentarRenda ? '🚨 Alerta de Renda' : '💡 Tipo de Ciclo'}
      </div>
      <h3 style={{ fontSize: 17, fontWeight: 800, color: '#0f0f0f', marginBottom: 10, lineHeight: 1.3 }}>{titulo}</h3>
      <p style={{ fontSize: 14, color: '#525252', lineHeight: 1.7, marginBottom: precisaAumentarRenda ? 16 : 0 }}>{corpo}</p>
      {precisaAumentarRenda && (
        <div style={{ display: 'flex', gap: 10 }}>
          <div style={{ flex: 1, background: '#fef2f2', border: '1px solid #fecaca', borderRadius: 10, padding: '14px 12px', textAlign: 'center' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', color: '#a3a3a3', fontSize: 18, fontWeight: 700 }}>→</div>
          <div style={{ flex: 1, background: '#f0fdf4', border: '1px solid #bbf7d0', borderRadius: 10, padding: '14px 12px', textAlign: 'center' }}>
      ...
      {precisaAumentarRenda && (
        <div style={{ marginTop: 12, background: '#f9f8f6', borderRadius: 10, padding: '12px 14px', display: 'flex', alignItems: 'center', gap: 10 }}>
          <span style={{ fontSize: 16 }}>💡</span>
          <p style={{ fontSize: 13, color: '#525252', lineHeight: 1.5 }}>
            O Método 6 Caixas mostra como estruturar sua renda atual <strong>e</strong> como criar novas fontes de receita.
          </p>
        </div>
      )}
    </div>
...
function BadgeRisco({ risco }) {
  return (
    <div style={{ display: 'inline-flex', alignItems: 'center', gap: 8, background: risco.fundo, border: `1px solid ${risco.borda}`, borderRadius: 9999, padding: '6px 16px' }}>
      <div style={{ width: 8, height: 8, borderRadius: '50%', background: risco.cor, animation: risco.nivel === 'vermelho' ? 'pulseRed 1.5s ease infinite' : 'none' }}/>
...
function CardMetrica({ label, valor, corValor }) {
  return (
    <div style={{ background: '#ffffff', border: '1px solid #e5e5e5', borderRadius: 12, padding: '20px 18px', flex: 1, boxShadow: '0 1px 3px rgba(0,0,0,0.05)' }}>
  ...
  return (
    <div style={{ background: '#ffffff', border: '1px solid #e5e5e5', borderRadius: 12, padding: '20px', boxShadow: '0 1px 3px rgba(0,0,0,0.05)' }}>
      ...
      <div style={{ height: 8, background: '#f5f5f5', borderRadius: 4, overflow: 'hidden' }}>
        <div style={{ height: '100%', borderRadius: 4, width: `${width}%`, background: percentual > 95 ? '#dc2626' : percentual > 85 ? '#d97706' : '#16a34a', transition: 'width 1.2s cubic-bezier(0.4,0,0.2,1)' }}/>
      </div>
...
function CardComposicaoGastos({ totalFixos, totalCartao, totalVariaveis, totalGastos }) {
  ...
  return (
    <div style={{ background: '#ffffff', border: '1px solid #e5e5e5', borderRadius: 16, padding: '24px 20px', boxShadow: '0 1px 3px rgba(0,0,0,0.05)', marginBottom: 16 }}>
      <div style={{ fontSize: 11, fontWeight: 800, color: '#f97316', letterSpacing: '0.1em', textTransform: 'uppercase', marginBottom: 8 }}>Composição dos Gastos</div>
      <h3 style={{ fontSize: 16, fontWeight: 800, color: '#0f0f0f', marginBottom: 4 }}>De onde vem o que você gasta</h3>
      <p style={{ fontSize: 13, color: '#737373', marginBottom: 16 }}>Total: {formatarMoeda(totalGastos)}</p>
          ...
          return (
            <div key={i}>
              ...
              <div style={{ height: 6, background: '#f5f5f5', borderRadius: 3, overflow: 'hidden' }}>
                <div style={{ height: '100%', borderRadius: 3, width: `${pct}%`, background: item.cor, opacity: item.valor > 0 ? 0.85 : 0.2, transition: 'width 1s cubic-bezier(0.4,0,0.2,1)' }}/>
              </div>
            </div>
          )
        })}
...
function CardCaixas({ analise6caixas, receita }) {
  ...
  return (
    <div style={{ background: '#ffffff', border: '1px solid #e5e5e5', borderRadius: 16, padding: '24px 20px', boxShadow: '0 1px 3px rgba(0,0,0,0.05)', marginBottom: 16 }}>
      <div style={{ fontSize: 11, fontWeight: 800, color: '#f97316', letterSpacing: '0.1em', textTransform: 'uppercase', marginBottom: 8 }}>Diagnóstico das 6 Caixas</div>
      <h3 style={{ fontSize: 16, fontWeight: 800, color: '#0f0f0f', marginBottom: 4 }}>Onde seu dinheiro está — vs onde deveria estar</h3>
      <p style={{ fontSize: 13, color: '#737373', marginBottom: 16 }}>Com base nos seus lançamentos de {formatarMoeda(receita)}/mês:</p>
      {(zeradas > 0 || risco > 0) && (
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
        {analise6caixas.map((c, i) => (
          <div key={i} style={{ background: c.fundo, border: `1.5px solid ${c.borda}`, borderRadius: 12, padding: '14px 16px' }}>
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 6 }}>
              ...
              <div style={{ background: c.cor + '20', border: `1px solid ${c.cor}40`, borderRadius: 6, padding: '3px 8px', fontSize: 11, fontWeight: 700, color: c.cor, whiteSpace: 'nowrap' }}>{c.label}</div>
            </div>
            <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
              <div style={{ flex: 1 }}>
                ...
                <div style={{ height: 6, background: '#e5e5e5', borderRadius: 3, overflow: 'hidden', position: 'relative' }}>
                  ...
                  <div style={{ position: 'absolute', height: '100%', width: `${Math.min(c.realPct, 100)}%`, background: c.cor, borderRadius: 3, opacity: 0.85 }}/>
                </div>
                <div style={{ display: 'flex', justifyContent: 'space-between', marginTop: 3 }}>
                  <span style={{ fontSize: 11, fontWeight: 700, color: c.cor }}>{c.realValor > 0 ? formatarMoeda(c.realValor) : 'R$ 0'}</span>
                  <span style={{ fontSize: 11, color: '#a3a3a3' }}>ideal: {formatarMoeda(c.idealValor)}</span>
                </div>
              </div>
            </div>
            {c.alerta && <div style={{ marginTop: 8, fontSize: 12, color: c.cor, fontWeight: 600, lineHeight: 1.4 }}>→ {c.alerta}</div>}
          </div>
        ))}
      </div>
    </div>
...
function BlocoCTA({ d, onIrParaMetodo }) {
  ...
  const [email, setEmail] = useState(() => localStorage.getItem('email_lead_diagnostico') || '')
  ...
  async function salvarEmail(e) {
    ...
    if (cleanEmail.includes('<') || cleanEmail.includes('>') || cleanEmail.includes('script')) {
    ...
    try {
      ...
      if (response.ok) {
        localStorage.setItem('email_lead_diagnostico', cleanEmail)
        setEmailSalvo(true)
  ...
  return (
    <div style={{ background: '#0f0f0f', border: d.ctaDestaque ? '2px solid #dc2626' : 'none', borderRadius: 20, padding: '28px 24px', marginBottom: 12, boxShadow: d.ctaDestaque ? '0 0 32px rgba(220,38,38,0.2)' : 'none' }}>
      {d.ctaDestaque && (
        <div style={{ display: 'inline-flex', alignItems: 'center', gap: 6, background: '#7f1d1d', borderRadius: 9999, padding: '4px 12px', marginBottom: 12 }}>
          <span style={{ fontSize: 11, fontWeight: 800, color: '#fca5a5', letterSpacing: '0.08em', textTransform: 'uppercase' }}>🚨 Passo zero identificado</span>
        </div>
      )}
      {!d.ctaDestaque && (
      ...
      <h3 style={{ fontSize: 20, fontWeight: 800, color: '#ffffff', marginBottom: 10, lineHeight: 1.3 }}>{d.ctaTitulo}</h3>
      <p style={{ fontSize: 14, color: '#a3a3a3', lineHeight: 1.65, marginBottom: 24 }}>{d.ctaTexto}</p>

      {!mostrarEmail ? (
        <button
          onClick={() => setMostrarEmail(true)}
          style={{ width: '100%', padding: '16px 24px', background: corBotao, color: '#ffffff', borderRadius: 12, fontSize: 16, fontWeight: 700, display: 'flex', alignItems: 'center', justifyContent: 'space-between', boxShadow: `0 4px 14px ${corSombra}`, transition: 'all 0.2s', border: 'none', cursor: 'pointer' }}
        ...
        <form onSubmit={salvarEmail} style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
          ...
          <input
            ...
            disabled={enviando}
            style={{ padding: '14px 16px', background: erroEmail ? '#2a0808' : '#1a1a1a', border: `1.5px solid ${erroEmail ? '#dc2626' : '#2c2c2c'}`, borderRadius: 10, fontSize: 15, color: '#ffffff', outline: 'none' }}
          />
          {erroEmail && (
            <div style={{ fontSize: 12, color: '#f87171', fontWeight: 600, display: 'flex', alignItems: 'center', gap: 6 }}>
              <span>⚠️</span> {erroEmail}
            </div>
          )}
          <div style={{ display: 'flex', gap: 8 }}>
            <button type="button" onClick={onIrParaMetodo} disabled={enviando} style={{ flex: 1, padding: '13px', borderRadius: 10, background: 'transparent', border: '1px solid #2c2c2c', color: '#737373', fontSize: 13, fontWeight: 600, cursor: 'pointer' }}>
              Pular
            </button>
            <button type="submit" disabled={enviando} style={{ flex: 2, padding: '13px', borderRadius: 10, background: corBotao, color: '#ffffff', fontSize: 14, fontWeight: 700, opacity: enviando ? 0.7 : 1, cursor: 'pointer', border: 'none', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8 }}>
              {enviando ? 'Enviando...' : 'Acessar o Método →'}
            </button>
          </div>
        </form>
      )}
...
export default function TelaDiagnostico({ respostas, onReiniciar }) {
  ...
  return (
    <div style={{ minHeight: '100vh', background: '#f9f8f6' }}>
      <header style={{ background: '#ffffff', padding: '16px 24px', borderBottom: '1px solid #e5e5e5', display: 'flex', alignItems: 'center', justifyContent: 'space-between', position: 'sticky', top: 0, zIndex: 10 }}>
      ...
      <main style={{ maxWidth: 600, margin: '0 auto', padding: '32px 20px 60px', animation: 'fadeUp 0.5s ease forwards' }}>
        <div style={{ background: '#ffffff', border: `2px solid ${d.risco.borda}`, borderRadius: 20, padding: '32px 24px', textAlign: 'center', marginBottom: 16, position: 'relative', overflow: 'hidden', boxShadow: '0 4px 12px rgba(0,0,0,0.06)' }}>
          ...
          <div style={{ fontSize: 'clamp(72px, 20vw, 100px)', fontWeight: 900, color: d.risco.cor, lineHeight: 1, letterSpacing: '-0.04em', marginBottom: 4, animation: animouDias ? 'countUp 0.5s ease forwards' : 'none' }}>{d.diasTexto}</div>
        ...
        <div style={{ background: '#ffffff', border: '1px solid #e5e5e5', borderRadius: 16, padding: '24px 20px', marginBottom: 16, boxShadow: '0 1px 3px rgba(0,0,0,0.05)' }}>
        ...
        <CardComposicaoGastos totalFixos={d.totalFixos} totalCartao={d.totalCartao} totalVariaveis={d.totalVariaveis} totalGastos={d.totalGastos} />

        <div style={{ marginBottom: 16 }}>
          <CardCaixas analise6caixas={d.analise6caixas} receita={d.receita} />
        </div>

        <div style={{ background: d.risco.fundo, border: `1px solid ${d.risco.borda}`, borderRadius: 12, padding: '16px 20px', marginBottom: 24, display: 'flex', gap: 12, alignItems: 'flex-start' }}>
          <span style={{ fontSize: 20, flexShrink: 0 }}>{d.risco.nivel === 'verde' ? '✅' : d.risco.nivel === 'amarelo' ? '⚠️' : '🚨'}</span>
        ...
        <button
          onClick={onReiniciar}
          style={{ width: '100%', padding: '13px', background: 'transparent', color: '#a3a3a3', fontSize: 13, borderRadius: 10, border: '1px solid #e5e5e5', transition: 'color 0.15s', cursor: 'pointer' }}
